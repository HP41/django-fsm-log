...
class Article(models.Model):
    ...
    def submit(self, by=None):
        self.django_fsm_log_description = "Submitted to approval"
        pass

    @fsm_log_by
    @transition(field=state, source='submitted', target='draft')
    def request_changes(self, by=None):
        self.django_fsm_log_description = "Put back in draft mode by {}".format(by.username)
        pass

    @fsm_log_by
    @transition(field=state, source='submitted', target='published')
    def publish(self, by=None):
        self.django_fsm_log_description = "Approved and published"
        pass
